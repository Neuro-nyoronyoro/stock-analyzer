...
st.divider()
with st.expander("📖 スクリーニング指標・用語の説明"):
    st.markdown("""
#### フィルター条件の指標

| 用語 | 正式名称 | 意味 | 目安 |
|------|----------|------|------|
| **配当利回り** | Dividend Yield | 株価に対して1年間に受け取れる配当の割合。年間配当金 ÷ 株価 × 100 で計算。 | 日本株平均2〜3%。高いほど収入が多い |
| **PER** | 株価収益率（Price Earnings Ratio） | 株価が1株あたり利益の何倍かを示す。低いほど割安。 | 日本株の目安15〜20倍。業種差が大きい |
| **PBR** | 株価純資産倍率（Price Book-value Ratio） | 株価が1株あたり純資産の何倍か。1倍未満は解散価値より安い状態。 | 1倍前後が割安の目安 |
...
|--------|-------------------|
| **配当スコア** | 配当利回りの高さ・安定性 |
| **財務スコア** | ROE・負債比率・流動比率などの財務健全性 |
| **成長スコア** | 売上高・利益の成長率 |
| **割安スコア** | PER・PBRの低さ（市場での割安度） |
| **総合スコア** | 「プロフィール」で設定した投資スタイルの重みで合算した総合評価。100点満点 |

> 💡 「アカウント設定」ページで投資スタイル（配当重視・成長重視など）を設定すると、あなたの方針に合ったスコアで銘柄を並び替えられます。
""")
