...
以下の複数銘柄データをもとに、「銘柄間の横断的な比較分析」に特化したレポートを日本語で作成してください。
各銘柄の個別説明は不要です。銘柄を並べて比べることでわかる相対的な差異・優劣・関係性に焦点を当ててください。
...
以下の観点で横断比較してください（各銘柄の単体説明は省き、比較・対比・序列を中心に記述）：

1. **割安度の比較**（PER・PBR）
   どの銘柄が相対的に割安か、逆に割高かを数値根拠とともに述べる

2. **収益性・財務健全性の比較**（ROE・負債比率）
   稼ぐ力と財務安定性で銘柄間にどのような差があるか

3. **配当・成長性のトレードオフ比較**
   配当利回りと成長率の観点で、どの銘柄がどちらに傾いているか

4. **投資家プロフィールへの適合度ランキング**
   上記スタイル・リスク・期間を踏まえ、最も適している銘柄から順に順位付けし、その理由を簡潔に述べる
